...
- **CI / review state:** for open PRs, `gh pr checks <n>` — call out anything red or anything
  waiting on the developer's review (especially auth/crypto PRs, which need a deliberate second
  review and must not be self-merged).
...
- **Andrea** — backend / API / auth / DB (`backend/`). Auth & crypto files
  ...
  need a deliberate second review — no self-merge.
