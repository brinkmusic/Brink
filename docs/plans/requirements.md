...
| UI-3 | Reactions call BE-5; counts reflect server truth. | T41, T97 | ✅ (T97 adds the double-tap-to-heart gesture on song cards — add-only, reuses the same `react()` path; no API change.) |
