...
- `047`, `016`, `053`, `063`, `003` (enablement wave, above) — no blockers.
- **Done:** `015` — user search API (`GET /api/users/search?q=`, login-gated + rate-limited,
  ILIKE on handle/display name, cap 20). `046` (search UI) now waits only on `047`.
